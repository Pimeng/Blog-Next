...
> [!IMPORTANT] 重要
> 如果使用真机，请别忘了电脑和手机需要保持在一个 WiFi/网络 下

### 根证书配置

> [!IMPORTANT] 重要
> 这一步很重要，因为现在大多数的软件都是使用 https 协议进行通信的，如果不安装根证书，抓包工具是无法解密 https 的请求的
...
然后在APP上进行登录操作，此时能看到登录的链接，我们点进去，找到原始，就能看到APP是如何请求服务器的

![请求详情](reqable-desktop-2.png)

这里就有请求的 `邮箱` `密码` ，然后还有返回的 `token` `refresh_token` 等等

## 0x04 收尾

更多细节旧不展开细说了，点到为止，更多内容自己挖掘即可